...
public class View extends JFrame {
	...
	Objects itrashbin=new Objects(ObjectSprites.TRASHBIN, 800, 620, false, true, 0, null);
	...
	private class OverPanel extends JPanel {
		 ...
		 * player can choose to restart the game (then it will jump back to
		 * where the game ended) or exit the game
		 * 
		 * @param g-
		...
		protected void paintComponent(Graphics g) {
			...
			this.setLayout(null);
			button2.setBounds(650, 200, 100, 50);
     ...
     */
		protected void paintComponent(Graphics g) {
			...
			button8.setBounds(530, 700, 100, 50);
			button9.setBounds(690, 700, 100, 50);
			button10.setBounds(870, 700, 100, 50);
			button11.setBounds(1200, 700, 100, 50);
	...
	private class DrawPanel extends JPanel {
		 ...
		 * according to different values of the playerRoom, and pain the view
		 * accordingly
		 * 
		 * @param g-
		 *            used to draw images on the screen
		...
		protected void paintComponent(Graphics g) {
    		...
    		if(!(imower.interacted) && p.xloc <= 500) {
    			g.drawImage(lawnmower_txt, 600, 230, null);
			...
			if(p.getPlayerRoom()==2) {
				...
				if ((yloc - 50 > p.getYloc()) && bt != 0) {
					beach = true;
					for (int i = 0; i < currentLength; i++) {
						items[p.getPlayerRoom()][i].setYloc(250);
					}
				}
			...
			if(iperson.interacted && p.getPlayerRoom() == 3) {
				...
				if(!ishirt.interacted) {
					...
					g.drawString(("$" + (100 - p.getHealth())), 350, 260);
				}
			}
		...
		/**
		 * This method checks if the player has replayed the game. If the player
		 * replays the game, it will reset the timer
		 * 
		 * @param reset-
		 *            a boolean value that indicates if the player replays the
		 *            game or not
		 * @return null
		 */
	...
	public void setT(boolean reset) {
		if (reset)
			startTime = System.currentTimeMillis();
	}
	/**
	 * This method is used to create the timer with texts indicating how much
	 * time left
	 * 
	 * @param The
	 *            string that will be displayed with the count-down timer
	 * @return The image with the count down timer and the string texts
	 */
	 ...
	 * has changes, it will be reflected on the screen
	 * @param interact 
	 * @param doorInteract 
	 * 
	 * @param xloc-
	 *            the lateral location of the player
	 * @param yloc-
	 *            the vertical location of the player
	 * @param d-
	 *            the direction of the player moving towards
	 * @param move-
	 *            the state that indicates if the player is moving or not
	 * @param interact-
	 *            the state that indicates if the player is interacting with an
	 *            object or not
	 * @param sprite-
	 *            the object that the player is interacting with
	 * @return null
	 */
	 ...
	 * 
	 * @param p-
	 *            the instance of the player object in the view
	 * @return null
	 */
	 ...
	 * 
	 * @param doorMap-
	 *            the 2D array of doors that will be displayed on the screen
	 * @return null
	 */
