...
class hashNode{
	...
	string *values;//// the dynamically allocated array of words that follow the key in the Dr. Seuss text.  If this set of words gets to be larger than the valuesSize of the array, then you must re-allocate the array double in size and copy over all the old values
	int valuesSize;//the size of the value array
...
public:
	...
	hashNode(string s); // initializes keyword to s, the arraysize to 100 (or whatever you like for starting), the currSize to 0, and the valuesSize to 100
